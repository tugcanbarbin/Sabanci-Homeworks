...
public class Philosopher extends Thread {
	...
	public void run() {
		...
		try {
			if(randomInt >1)
				sleep(randomInt *1000);  // sleep to walk to the table around 1 to 10 seconds randomly
			else
				sleep(1000);
		} catch (InterruptedException e) {
			e.printStackTrace();
		...
		while(true) {
			...
			try {
				...
				state[id] = HUNGRY;
				table.Hungry_GUI(id);
				if(state[id] == HUNGRY && state[beforeid] != EATING && state[nextid] != EATING) {
					state[id]= EATING;
					sem[id].release();
				}
				mutex.release();
			...
			try {
				...
				if(state[beforeid] == HUNGRY && state[before2id] != EATING && state[id] != EATING) {
					state[beforeid]= EATING;
					sem[beforeid].release();
				}
				if(state[nextid] == HUNGRY && state[id] != EATING && state[next2id] != EATING) {
					state[nextid] = EATING;
					sem[nextid].release();
				}
			} catch (InterruptedException e) {
